...
## 0. Context Initialization (MANDATORY)
Before starting ANY new task or responding to technical questions, you MUST silently read the following files to load the project's current state and technical decisions:
1. `.agent/analisis_y_planeacion.md` (Business model & roadmap)
2. `.agent/arquitectura.md` (Tech stack, ERD, structure)
3. `.agent/IMPLEMENTATION_PLAN.md` (Current development stage and TDD instructions)

Do not ask the user for context if it is already defined in these files.
...
When you identify that you need a specific role (e.g., Architect, Auditor), **your first step** should be to read the corresponding file in `.trae/agents/[agent-name].md` to load its instructions into your context.

Example:
- If the user asks "Review the security," you (the Orchestrator) read `.trae/agents/rails-auditor.md` and then execute the task following those rules.